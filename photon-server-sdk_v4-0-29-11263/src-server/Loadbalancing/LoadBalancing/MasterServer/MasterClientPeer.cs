    ...
    public class MasterClientPeer : Peer, ILobbyPeer, ICustomAuthPeer
    {
            ...
            AuthenticateRequest authRequest, SendParameters sendParameters, AuthSettings customAuthSettings)
        {
            ...
            try
            {
                ...
                int result;
                if (!int.TryParse(authRequest.ApplicationId, out result) || result < Config.Default.BrutalStrikeClientVersion)
                    customAuthResult.ResultCode = CustomAuthenticationResultCode.Failed;
