...
//         if (!owner?.stats?.android == false && !running && isGrounded) //if android
//             vel = mv * maxSpeed * slowDown * slowDown; //pr.MaxSpeed(playerState)*totalWeightEvaluted2;
//         else
//         {
...
//
//         if (controller.noclip /*||!controller.enableOverlapRecovery */ || pr.flying)
//             gVel = Vector3.zero;
//         else if (ControllerIsGrounded && gVel.y < .1f) //to fix jump stuck
//             gVel = gravity * 20;
//         else
//             gVel += gravity;
//
...
//         }
//         if (roomSettings.enableSliding && roomSettings.SlideSpeed > 0 && isGrounded && sliding)
//             mVel += Vector3.Cross(Vector3.Cross(ccHit.normal, Vector3.down), ccHit.normal).normalized * roomSettings.SlideSpeed;
//         
//         // if (!IsMine)
